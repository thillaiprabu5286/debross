...
class Owner_About {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'owner-about-theme', get_template_directory_uri() . '/inc/about-theme/about.css', array(), $owner_version );
	}
	...
	public static function hide_notices() {
		if ( isset( $_GET['owner-hide-notice'] ) && isset( $_GET['_owner_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_owner_notice_nonce'], 'owner_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'owner' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'owner' ) );
			}
	...
	public function welcome_notice() {
		?>
		<div id="message" class="updated owner-message">
			...
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing owner! To fully take advantage of the best our theme can offer please make sure you visit our %1$s welcome page %1$s.', 'owner' ), '<a href="' . esc_url( admin_url( 'themes.php?page=owner-welcome' ) ) . '">', '</a>' ); ?></p>
			<p class="submit">
				<a class="button-secondary" href="<?php echo esc_url( admin_url( 'themes.php?page=owner-welcome' ) ); ?>"><?php esc_html_e( 'Get started with owner', 'owner' ); ?></a>
			</p>
		</div>
		<?php
	}
	...
	private function intro() {
		...
		<div class="owner-theme-info">
				<h1>
					<?php esc_html_e('About', 'owner'); ?>
					<?php echo $theme->display( 'Name' ); ?>
					<?php printf( '%s', $owner_version ); ?>
				</h1>
			...
			<div class="changelog point-releases">
				<div class="under-the-hood two-col">
					<div class="col">
						...
						<p><a href="<?php echo admin_url( 'customize.php' ); ?>" class="button button-secondary"><?php esc_html_e( 'Customize', 'owner' ); ?></a></p>
					</div>
						...
						foreach ( $themes->themes as $theme ) {
							if( $active_theme == $theme->name ) { ?>
								...
								<div id="<?php echo $theme->slug; ?>" class="theme active">
									...
									<h3 class="theme-name" id="owner-name"><strong><?php _e( 'Active', 'owner' ); ?></strong>: <?php echo $theme->name; ?></h3>
									<div class="theme-actions">
										<a class="button button-primary customize load-customize hide-if-no-customize" href="<?php echo get_site_url(). '/wp-admin/customize.php' ?>"><?php _e( 'Customize', 'owner' ); ?></a>
									</div>
								</div><!-- .theme active -->
							<?php
							$counter++;
									...
									<div class="theme-actions">
										<?php if( wp_get_theme( $theme->slug )->exists() ) { ?>
											...
											<a  class="button button-secondary activate"
												href="<?php echo wp_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . urlencode( $theme->slug ) ), 'switch-theme_' . $theme->slug );?>" ><?php _e( 'Activate', 'owner' ) ?></a>
										<?php } else {
										...
										?>
											<!-- Install Button -->
											<a data-toggle="tooltip" data-placement="bottom" title="<?php echo 'Downloaded ' . number_format( $theme_details->downloaded ) . ' times'; ?>" class="button button-secondary activate" href="<?php echo esc_url( wp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ); ?>" ><?php _e( 'Install Now', 'owner' ); ?></a>
										<?php } ?>

										<a class="button button-primary load-customize hide-if-no-customize" target="_blank" href="<?php echo $theme->preview_url; ?>"><?php _e( 'Live Preview', 'owner' ); ?></a>
									</div>
								</div><!-- .theme -->
								<?php
							}
