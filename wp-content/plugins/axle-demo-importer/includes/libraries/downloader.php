...
class ADI_Downloader {
	...
	public function download_file( $url, $filename = '' ) {
		if ( empty( $filename ) ) {
			$file_basename = pathinfo( $url, PATHINFO_BASENAME );
			$filename = sanitize_file_name( $file_basename );
		}

		// Bail if filename is not valid.
		if ( empty( $filename ) ) {
			return new WP_Error(
				'invalid_file_name_in_url',
				__( 'Invalid file name.', 'axle-demo-importer' )
			);
		}
		...
		return $this->write_to_file( $content, $this->download_directory_path . $filename );
	}
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'axle-demo-importer' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'axle_demo_impoter_download_timeout', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'axle-demo-importer' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
				...
				apply_filters( 'axle_demo_impoter_message_after_file_fetching_error', '' )
			);
		}
	...
	public function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'axle-demo-importer' ),
					'<br>',
					$file_path
				)
			);
